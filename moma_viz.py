...
con = psycopg2.connect("dbname=moma user=postgres host=localhost port=5433")
...
def task1():
    ...
    query =  """ 
        ...
        SELECT DISTINCT department, COUNT(department) as count FROM moma_works
        GROUP BY department
        GROUP BY count DESC;
...
def task2():
    ...
    query = """ 
        ...
        SELECT DISTINCT classification, COUNT(classification) as count FROM moma_works
...
def task3():
    ...
    query = """
        ...
        SELECT 
        DISTINCT (INFO -> 'nationality') AS nationality,
        COUNT(info-> 'nationality') AS count
        FROM moma_artists
        WHERE (info -> 'nationality' is 'null')
...
def task4():
    ...
    query = """
        ...
        SELECT 
        Count(info -> ('Male')) as male_cnt,
        Count(info -> ('Female')) as female_cnt,
        Count(info -> ('Non-binary')) as nb_cnt
...
def task5():
    ...
    title = "New Artists Signed Per Day"  # TODO task 5
    query = """
